...
export default function ProductDetails() {
  ...
  return (
        ...
        <View className='relative h-[450px] bg-gray-100 mb-6'>
            <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} scrollEventThrottle={16}
                ...
                {product.images.map((image, index) => (
                    <Image key={index} source={{uri: image}} style={{width: width, height: 650}} resizeMode='cover' />
                ))}
            ...
            <View className='px-5'>
                ...
                <View className='flex-row justify-between items-start mb-2'>
                    <Text className='text-xl font-bold text-primary flex-1 mr-4'>{product.name}</Text>
                    <View className='flex-row justify-between items-start mb-2'>
                        <Ionicons name='star' size={14} color='#FFD700' />
                        <Text className='text-sm font-bold ml-1'>4.6</Text>
                        <Text className='text-sm font-bold ml-1'>(120)</Text>
                    </View>
                </View>
                ...
                {product.sizes && product.sizes.length > 0 && (
                    <View className='mb-6'>
                        <Text className='text-lg font-bold text-primary mb-2'>Sizes</Text>
                        <View className='flex-row flex-wrap gap-2'>
                            {product.sizes.map((size) => (
                                <TouchableOpacity
                                    key={size}
                                    onPress={() => setSelectedSize(size)}
                                    className={`px-4 py-2 border rounded-full ${selectedSize === size ? 'bg-primary border-primary' : 'border-gray-300'}`}
                                >
                ...
                {/* description */}
                <View className='mb-6'>
                    <Text className='text-lg font-bold text-primary mb-2'>Description</Text>
    ...
    {/* footer */}
    <View className='absolute bottom-8 left-0 flex-row right-0 p-4 bg-white border-t border-gray-100'>
