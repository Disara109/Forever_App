...
export default function Home() {
  ...
  const [activeBannerIndex, setActiveBannerIndex] = useState(0);// managed with state to reflect the current banner.

  const [products, setProducts] = useState<Product[]>([]); // for future use when fetching products based on category.
  
  const [loading, setLoading] = useState(true); // to manage loading state when fetching products.

  const categories = [{ id: 'all', name: 'All', icon: "grid" }, ...CATEGORIES];

  const fetchProducts = async ()=> {
    setProducts(dummyProducts); // Simulating an API call with dummy data.
    setLoading(false);
  }
  ...
  return (
    <SafeAreaView className='flex-1' edges= {['top']}>
      ...
      <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} className='w-full h-48 rounded-xl'
          ...
          <View key={index} className='relative w-full h-48 bg-gray-200 overflow-hidden' style={{ width: width - 32 }}>
            ...
            <View className='absolute bottom-4 left-4 z-10'>
              ...
              <TouchableOpacity className='mt-2 bg-white px-4 py-2 rounded-full self-start'>
                <Text className='text-primary font-bold text-xs text-center'>Shop Now</Text>
              </TouchableOpacity>
            </View>
      ...
      <View className='flex-row justify-center mt-3 gap-2'>
        {BANNERS.map((_, index) => (
          <View key={index} className= {`h-2 rounded-full ${index === activeBannerIndex ? 'w-6 bg-primary' :
            'w-2 bg-gray-300' }`} />
        ))}
      </View>
