...
//GET /api/products
 export const getProducts = async (req: Request, res: Response) => {
    try {
        ...
        const products = await Product.find(query).skip((Number(page) - 1) * 
    ...
    try {
            ...
            const uploadPromises = (req.files as any).map((file: any)=> {
                return new Promise((resolve, reject)=> {
                    const uploadStream = cloudinary.uploader.upload_stream({folder:
                        'forever-app/products'}, (error, result)=> {
        ...
        if(typeof sizes == "string") {
            try {
                ...
                sizes = sizes.split(",").map((sizes: string)=> sizes.trim()).filter((s: string)=> s !== "");
            }
        }
        //ensure the arrays
